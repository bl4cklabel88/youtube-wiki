...
- CAPTURE DIAGNOSTIC TRICKS OR SHORTCUTS: Technicians often use "tricks" or undocumented shortcuts (e.g., bypassing a sensor with a jumper wire while watching scan data instead of following the manual's pin-by-pin resistance chart). Always extract and clearly document these real-world shortcuts under the method steps or key insights.
- The when_to_use value must be a single string containing bullet points or descriptive text, NOT a JSON array.
