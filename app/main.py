...
def index(request: Request, q: str = Query(None), category: str = Query(None), tag: str = Query(None)):
    ...
    from .wiki.models import Category, Channel
...
def admin_dashboard(request: Request):
    ...
    # Template expects a dict for video_stats.items()
    video_stats_dict = {row["status"]: row["c"] for row in videos_stats}

    return templates.TemplateResponse(request=request, name="admin.html", context=_ctx(request, **{
        "sources": sources,
        "jobs": jobs,
        "drafts": drafts,
        "video_stats": video_stats_dict,
